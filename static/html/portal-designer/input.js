...
function generateModelCases() {
	...
	var increment_on_pitch = 0.1; // in m
...
function generateModelCaseTable () {
	...
	for (var i = 0; i < model_cases_parameters.length; i++) {
		...
		var this_case_btn = '<i class="calculator icon" id='+case_id +' onClick="testSolveModelCase(model_cases['+i+'],'+i+')"></i> <i class="check circle icon disabled" id='+case2_id +' onClick="testDesignCheck(model_cases['+i+'],'+i+')"></i>';
...
jQuery(document).ready(function() {
	...
	jQuery("#generate-model-cases-btn").click(function () {
		generateModelCases();
	})

	jQuery("#animate-cases-btn").click(function(){
		if (model_cases.length == 0) generateModelCases();
		animateCases("fast");
	})
})